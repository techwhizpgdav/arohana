    ...
    <div className='mb-20'>
      <div className="flex justify-around items-center mt-8">
        {rounds.map(round => (
          <button
            key={round.id}
            className={`bg-haldi-orange  text-white font-bold py-3 px-14 rounded mx-2 transition duration-300 ease-in-out transform hover:scale-105 ${selectedRound === round?.id ? 'opacity-100' : ' opacity-50'}`}
          ...
          <div key={round?.id} className={selectedRound === round.id ? 'block' : 'hidden'}>
            <h2 className="text-xl mt-4 uppercase"> Mode: {round?.mode}</h2>
              ...
              round?.rules.map((rule, index) => (
                  <li className="mt-2 " key={index}>{rule.statement}</li>
              ))
              }</ul>
          </div>
        ))}
