...
const DesktopNav = () => {
  ...
  const color = "white";
  const bgcolor = isLoggedIn
    ? "bg-gradient-to-r from-haldi-orange to-haldi-red"
    : "bg-gradient-to-r from-haldi-yellow to-haldi";

  const navbarStyle = (path) =>
    `py-4 px-2 border-b-4 border-transparent text-${color} ${
      location.pathname === path ? "border-haldi-yellow" : ""
    } ${
      isLoggedIn
        ? `${isOnDashboard ? "navbar" : "hover:border-haldi"}`
        : "hover:border-haldi-orange"
    } font-medium rounded`;

  return (
    <nav
      className={` z-20 shadow-lg pt-2 pb-2 ${
        isOnDashboard
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-40 ">
            <div className="hidden md:flex items-center space-x-6">
              ...
              <Link data-aos="fade-up" to="/competitions">
                <p className={navbarStyle("/competitions")}>Events</p>
              </Link>
              <Link data-aos="fade-down" to="/timeline">
                <p className={navbarStyle("/timeline")}>Timeline</p>
              </Link>
              {/* Added Sponsors to Home Page */}
              {/* <Link data-aos="fade-down" to="/sponsors">
                <p className={navbarStyle("/sponsors")}>Sponsors</p>
              </Link> */}
              <Link data-aos="fade-up" to="/teams">
                <p className={navbarStyle("/teams")}>Teams</p>
              </Link>
            </div>
          </div>
                ...
                <Link to={"/dashboard/userProfile"}>
                  <div
                    ...
                    className={`py-2 px-2 font-medium rounded ${
                      isOnDashboard
                        ? " text-white hover:text-shade-darkBlue"
                        : " text-white hover:text-rose-200"
                    } duration-500 transition-all flex flex-col items-center`}
                  >
                    <img
                      src={UserProfile}
                      alt="UserProfile"
                      className={`h-8 w-8  logoWhite rounded-ful `}
                    />
                    <div className=" text-sm">Dash Board</div>
                  </div>
                </Link>
                <div className={`btn`}>
                  <button
                    data-aos="fade-left"
                    onClick={logout}
                    className={`btn-content-login font-medium ${
                      isOnDashboard
                        ? " text-white  hover:text-shade-darkBlue "
                        : " text-white hover:text-rose-200"
                    } `}
